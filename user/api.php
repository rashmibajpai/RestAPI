...
// echo "connected successfully.";   
	...
	if(isset($_GET['apicall'])){
		...
		switch($_GET['apicall']){
			...
			case 'signup':
				if(isTheseParametersAvailable(array('p_fname','p_lname','p_emailid','p_phone','password'))){
					...
					$stmt = $connect->prepare("SELECT p_id FROM patient WHERE p_emailid = ? OR p_phone = ?");
					$stmt->bind_param("si", $p_emailid, $p_phone);
					$stmt->execute();
					$stmt->store_result();
					...
					if($stmt->num_rows > 0){
						...
						$stmt = $connect->prepare("INSERT INTO patient (p_fname, p_lname,p_emailid, p_phone, password,p_status) VALUES (?, ?, ?, ?,?,'1')");
						$stmt->bind_param("sssis", $p_fname, $p_lname, $p_emailid, $p_phone, $password);
